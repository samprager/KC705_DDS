...
# commenting these as generic delays are mentioened below. These err out in SGMII modes.
#set_max_delay  -from [get_cells {*I_REGISTERS/IFGP_I/reg_data_reg[*]}]            -to [get_cells {*TX_IFGP_CROSS_I/Clk_B_BUS_OUT_reg[*]}] 32 -datapath_only
#set_max_delay  -from [get_cells {*I_REGISTERS/CR_I/reg_data_reg[*]}]              -to [get_cells {*RAF_REG_CROSS_I/Clk_B_BUS_OUT_reg[*]}] 32 -datapath_only

# It is possible that other set_max_delay definitions are required for optional VLAN/extended
# address filtering configuration options......
#set_false_path -from [get_ports {PCSPMA_STATUS_VECTOR[*]}] # May be needed in 1000BASE-X/SGMII mode or always optimised away in the system?

# ncsrama: adding a generic false path and a generic max delay  constraint for remaining paths
set_max_delay        -to [get_pins -hier -regexp {.*/Clk_B_BUS_OUT_reg.*D}] 24
